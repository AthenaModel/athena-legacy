...
#
# DESCRIPTION:
#    app_ingest(n): myserver(i) Server
#
...
snit::type appserver {
    ...
    typemethod init {} {
        ...
        appserver register /ingest/{c} {ingest/([[:alnum:]_.:]+)} \
            text/html [myproc /ingest:html]           \
            "Detail page for ingest model ingest {c}."

        appserver register /enum/snapshots {enum/snapshots}                   \
            tcl/enumdict [myproc /enum/snapshots:enumdict]                    \
            "Enumeration: Snapshots"

        appserver register /enum/sortingestby {enum/sortingestby}                 \
            tcl/enumlist [asproc enum:enumlist esortingestby]                   \
            tcl/enumdict [asproc enum:enumdict esortingestby]                   \
            text/html    [asproc type:html "Enum: Sort Cells By" esortingestby] \
        ...
        appserver register /page/{p} {page/(\w+)/?} \
            ...
            "Detail page for ingest model page {p}."
    }
    ...
    proc /:html {udict matchArray} {
        ...
        if {$code eq "unchecked"} {
            ht putln {
                The ingest model has not yet been checked.
        ...
        if {$code eq "syntax"} {
            ...
            ht putln "The ingest model has a syntax error at line "
        ...
        if {$code eq "checked"} {
            ht putln "The ingest model is sane, and contains the following pages:"
            ht para

            PageTable
        } else {
            ht putln "The ingest model is insane."
            ht para
        }


        # Referenced but not defined.
        if {[llength [cm ingests unknown]] > 0} {
            ht subtitle "Missing Cells"

            ht putln "The following ingests are referenced but not defined:"
            ht para

            ht ul {
                foreach ingest [cm ingests unknown] {
                    set refcount [llength [cm ingestinfo usedby $ingest]]
                    
                    ht li {
                        ht putln [format "%03d times: <b>%s</b>" $refcount $ingest]
                    }
                }
            }
        }
        ...
        if {[llength [cm ingests invalid]] > 0} {
            ht subtitle "Invalid Cells"

            ht putln "The following ingests have serious errors:"
            ht para

            foreach ingest [cm ingests invalid] {
                set line [cm ingestinfo line $ingest]

                ht link my://app/ingest/$ingest $ingest
                ht put " (Line "
                ht link gui://editor/$line $line
                ht put ")"
                ht put " = "
                ht put [normalize [FormulaWithLinks $ingest]]
                ht para 

                ht ul {
                    foreach message [CellErrors $ingest] {
                        ht li {
                            ht put $message
                        }
                    }
    ...
    # OverviewSane:html
    #
    # Outputs an overview of a sane ingest model
    ...
    #-------------------------------------------------------------------
    # /ingest/{c}:    Model page details
    #
    # {c} => $(1)  - The ingest name

    # /ingest:html udict matchArray
    #
    # Formats and displays the details for a particular model ingest.

    proc /ingest:html {udict matchArray} {
        upvar 1 $matchArray ""

        set ingest $(1)

        if {$ingest ni [cm ingests]} {
            if {[cmscript checkstate] in {unchecked syntax}} {
                ht page "Model Cell: $ingest" {}
                return [ht get]
            }
        ...
        set page [cm ingestinfo page $ingest]
        set bare [cm ingestinfo bare $ingest]
        ...
        ht table {"Line#" "Value" "Formula"} {
            ht tr valign top {
                ...
                ht td left { ht put [cm value $ingest] }
                ht td left {
                    ht put <code>
                    ht put [FormulaWithLinks $ingest] 
                    ht put </code>
                }
            }
        }
        ...
        # NEXT, put in errors.
        if {$ingest in [cm ingests invalid]} {
        ...
        if {[llength [cm ingestinfo uses $ingest]] > 0} {
            ht subtitle "$ingest uses these ingests:"
            CellTable [cm ingestinfo uses $ingest] model $page
            ht para
        }

        if {[llength [cm ingestinfo usedby $ingest]] > 0} {
            ht subtitle "$ingest is used by these ingests:"
            CellTable [cm ingestinfo usedby $ingest] model $page
            ht para
        }

        # NEXT, do ingests with this name appear on other pages.
        set ingests [list]
        foreach p [cm pages] {
            if {$p eq $page} {
                continue
            ...
            set pingests [cm ingests $p]

            if {$pingest in [cm ingests $p]} {
                lappend ingests $pingest
            }
        }

        if {[llength $ingests] > 0} {
            ht subtitle "Cells with the same name on other pages:"
            CellTable $ingests model 
            ht para
        }
    ...
    #
    # Links to the ingests on the page.
    ...
    proc /page:linkdict {udict matchArray} {
        ...
        foreach ingest [lsort -dictionary [cm ingests $page]] {
            dict set result my://app/ingest/$ingest \
                label $ingest 
    ...
    #
    #    sortby - The column to sort the ingests by: one of line, name.
    #
    ...
    proc /page:html {udict matchArray} {
        ...
        dict with qdict {
            ...
            restrict sortby esortingestby name
        }
        ...
        ht put " [llength [cm ingests $page]] ingests."
        ht putln "It is defined starting at line "
        ht link gui://editor/$pline $pline
        ht putln "in the ingest model file."
        ht para

        set ingests [cm ingests $page]

        if {$sortby eq "name"} {
            set ingests [lsort -dictionary $ingests]
        }

        CellTable $ingests $snapshot $page
    ...
    proc PageTable {} {
        ...
        ht table {"Line" "Page" "# of Cells" "Cyclic?"} {
            foreach page [cm pages] {
                set pcount [llength [cm ingests $page]]
    ...
    # CellTable ingests snapshot ?page?
    #
    # ingests    - List of ingests
    # snapshot - Snapshot from which to get ingests.
    # page     - Page whose name should be omitted from ingest names
    #
    # Adds an HTML table of ingest definitions and links to the output.
    # Cells are written with their fully-qualified names unless they
    # are on the named page.

    proc CellTable {ingests snapshot {page ""}} {
        ...
        ht table {"Line#" "Cell" "Value" "Formula"} {
            foreach c $ingests {
                ...
                set bare [cm ingestinfo bare $c]

                ht tr valign top {
                    ht td right {
                        set line [cm ingestinfo line $c]
                        ht link gui://editor/$line $line
                    }
                    ht td left {
                        if {[cm ingestinfo page $c] ne $page} {
                            ht link my://app/ingest/$c $c 
                        } else {
                            ht link my://app/ingest/$c $bare 
                        }
    ...
    # CellErrors ingest
    #
    # ingest   - A ingest with errors
    #
    # Returns a list of the error messages.

    proc CellErrors {ingest} {
        set out [list]

        if {[cm ingestinfo error $ingest] ne ""} {
            lappend out [normalize [cm ingestinfo error $ingest]]
        }

        foreach ringest [cm ingestinfo unknown $ingest] {
            lappend out "References undefined ingest: $ringest"
        }

        foreach ringest [cm ingestinfo badpage $ingest] {
            lappend out \
                "References ingest on later page: <a href=\"my://app/ingest/$ringest\">$ringest</a>"
        }

        return $out
    }

    # FormulaWithLinks ingest ?root?
    #
    # ingest   - A fully-qualified ingest name
    # root   - Root URL for ingest links
    #
    # Returns the ingest's formula with ingest links.  The ingests display
    # as themselves; the link URL is "${root}$ingest", allowing the 
    # routine to be used for more than one kind of link.

    proc FormulaWithLinks {ingest {root my://app/ingest/}} {
        # FIRST, if there's no such ingest then there's no formula.
        if {$ingest ni [cm ingests]} {
            return ""
        }

        # NEXT, get this ingest's page and formula
        set thisPage [cm ingestinfo page $ingest]
        set formula  [cm formula $ingest]
        ...
        foreach c [cm ingestinfo uses $ingest] {
            # The ingest might be referenced but not defined.
            if {$c ni [cm ingests]} {
                continue
            }
            ...
            if {$thisPage ne "null" &&
                [cm ingestinfo page $c] eq $thisPage
            } {
                set bare [cm ingestinfo bare $c]
