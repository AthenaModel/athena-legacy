#-----------------------------------------------------------------------
# TITLE:
#    strategyxbrowser.tcl
#
# AUTHORS:
#    Will Duquette
#
# DESCRIPTION:
#    strategyxbrowser(sim) package: Mark II Strategy Browser
#
# NOTIFICATIONS:
#
...
snit::widget strategyxbrowser {
    ...
    method MonitorApplication {} {
        ...
        notifier bind ::strategyx <Check>   $self [mymethod ReloadOnEvent]
        ...
        notifier bind ::strategyx <blocks>  $self [mymethod StrategyBlocks]
        ...
        notifier bind ::tacticx    <update> $self [list $ttab uid update]
        notifier bind ::conditionx <update> $self [list $ctab uid update]
    }
    ...
    method StrategyBlocks {op strategy_id block_id} {
        ...
        set s [strategyx get $strategy_id]
        ...
        switch -exact -- $op {
            ...
            delete {
                ...
                puts "oldSelection=$oldSelection, deleted=$block_id"
    ...
    method BlockUpdate {id} {
        ...
        # NEXT, if it's not our block we don't care.
    ...
    method BlockConditions {op block_id condition_id} {
        ...
        # NEXT, Update ctab.  Note that ctab already updates lazily, so we 
    ...
    method ReloadContent {{tag ""}} {
        ...
        if {$info(agent) ni [agent names]} {
            ...
            puts "---> ReloadContent setting agent to $info(agent)"
    ...
    method ReloadFirstBlock {} {
        ...
        puts "---> blocks=<$blocks>"
        puts "---> info(block)=<$info(block)>"
        if {[llength $blocks] == 0} {
            puts "---> clearing $info(block)"
            set info(block) ""
            $blist uid select {}
        } elseif {$info(block) ni $blocks} {
            set info(block) [lindex $blocks 0]
            puts "---> setting info(block) to <$info(block)>"
    ...
    method ReloadBList {} {
        ...
        $blist configure -beancmd [list [strategyx getname $info(agent)] blocks]
        $blist reload -force
    }
    ...
    method ReloadBlock {} {
        ...
        ::cond::simPP_predicate update \
            [list $ct_cmode $ct_addbtn $tt_emode $tt_addbtn]
    }
    ...
    method ReloadBlockMetadata {} {
        if {$info(block) ne ""} {
            $ct_cmode set [$info(block) get cmode]
            $tt_emode set [$info(block) get emode]
        } else {
            $ct_cmode set ""
            $tt_emode set ""
        }
    ...
    method AListAgentSelect {} {
        ...
        # TBD: Is there somewhere else this should be done?
        ...
        set info(block) ""
        puts "\nAlistAgentSelect $info(agent) from [caller]"
    ...
    method BListAdd {} {
        order send gui STRATEGY:BLOCK:ADD agent $info(agent)
    }
    ...
    method BMetaCreate {pane} {
        ...
        install bmeta using ttk::label $pane \
            -text "Block Metadata Controls"
    }
    ...
    # CTabCmode value
    #
    # value   - New cmode value
    #
    # Sets the block's condition mode.
    
    method CTabCmode {value} {
        # FIRST, ignore this if there's no block or there's no cmode
        # selected, or if this isn't a change.
        if {$value       eq "" || 
            $info(block) eq "" ||
            [$info(block) get cmode] eq $value 
        } {
            return
        }

        # NEXT, set the block's cmode.
    ...
    method CTabAdd {} {
        ...
        set tdict [conditionx titledict]
    ...
    method CTabEdit {} {
        ...
        set cond [conditionx get $id]

        # NEXT, allow editing.
        order enter CONDITIONX:[$cond typename]:UPDATE condition_id $id
    }
    ...
    method CTabState {} {
        ...
        if {$state eq "disabled"} {
            order send gui CONDITIONX:STATE condition_id $id state normal
        } else {
            order send gui CONDITIONX:STATE condition_id $id state disabled
        }
    }
    ...
    # TTabEmode value
    #
    # value   - New emode value
    #
    # Sets the block's execution mode.
    
    method TTabEmode {value} {
        # FIRST, ignore this if there's no block or there's no emode
        # selected or there's no change.
        if {$value       eq "" || 
            $info(block) eq "" ||
            [$info(block) get emode] eq $value 
        } {
            return
        }

        # NEXT, set the block's emode.
        order send gui BLOCK:UPDATE \
            block_id  [$info(block) id] \
            emode     $value
    }
    ...
    method TTabAdd {} {
        ...
        set tdict [tacticx titledict $atype]
        set titles [dict keys $tdict]
    ...
    method TTabEdit {} {
        ...
        set tactic [tacticx get $id]

        # NEXT, allow editing.
        order enter TACTICX:[$tactic typename]:UPDATE tactic_id $id
    }
    ...
    method TTabState {} {
        ...
        if {$state eq "disabled"} {
            order send gui TACTICX:STATE tactic_id $id state normal
        } else {
            order send gui TACTICX:STATE tactic_id $id state disabled
        }
    }
