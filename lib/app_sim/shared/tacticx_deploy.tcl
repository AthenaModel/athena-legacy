...
tacticx define DEPLOY "Deploy Personnel" {actor} -onlock {
    #-------------------------------------------------------------------
    # Instance Variables
    ...
    method obligate {coffer} {
        ...
        if {[strategyx ontick] && $totalTroops == 0} {
            return 0
        }
    ...
    method ObligateALL {coffer} {
        ...
        if {[strategyx locking] || $costPerPerson == 0.0} {
...
# TACTICX:DEPLOY:UPDATE
#
# Updates existing DEPLOY tactic.

order define TACTICX:DEPLOY:UPDATE {
    ...
    prepare tactic_id  -required -oneof [tacticx::DEPLOY ids]
    returnOnError
